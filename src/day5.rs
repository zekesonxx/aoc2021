...
impl Line {
	...
	// (bool, bool) -> is positive x, is positive y
...
pub fn part2(lines: &[Line]) -> usize {
	let maxx = lines.iter().map(|x| cmp::max(x.x1, x.x2)).max().unwrap()+1;
	let maxy = lines.iter().map(|x| cmp::max(x.y1, x.y2)).max().unwrap()+1;
	//  |
	//  y
	//  | 
	//   -x-
	...
	for line in lines {
		if line.is_v() {
			...
			let mut x = line.x1;
			let mut y = line.y1;
			let (xdir, ydir) = line.directions();
			loop {
				grid[x][y] += 1;
				if xdir { x += 1; } else { x -= 1; }
				if ydir { y += 1; } else { y -= 1; }
				if x == line.x2 {
					grid[x][y] += 1;
					break;
				}
